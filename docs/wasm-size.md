...
| 05-01 | real Sepolia USDC constant: `#[cfg(feature = "testnet")]` USDC constant switched from MockUsdc to the real Arbitrum Sepolia USDC (`0x75faf1…AA4d`); no logic change, only the embedded address literal and doc-comment text | 20,417 | 1 | -66 vs 02-02 (20,483), within the ~100-150 B brotli non-determinism band |
| CH1-01 | C-H1 gap-closure fix: `init(owner)` replaced with a real `#[constructor]` (matches `vault-periphery/src/router.rs`'s proven pattern); no post-deploy claimable entrypoint remains. Retired the now-unreachable `AlreadyInitialized`/`already_initialized()` error code (dead after the constructor removed its only caller) | 20,681 | 1 | +264 vs 05-01 (20,417); comfortably inside the documented brotli noise band plus the small real cost of the `#[constructor]` macro's own dispatch. Headroom under the 22,528-byte gate: 1,847 bytes (~8.2%). D-13 STOP rule did NOT trigger. |
| DROP-PERMIT2-01 | `deposit_for` demoted from a public ABI entry to a private helper (plain `impl` block) — `vault-periphery` is gone, so its only external caller is retired. `deposit(uint256)` stays public and delegates to the now-private `deposit_for`. No logic change, only the removed `#[public]`-block export | 20,395 | 1 | −870 vs CH1-01 (21,265, the post-WR-01/WR-04 pre-Phase-13 baseline in `docs/known-issues.md`) — dropping a public selector from the exported ABI shrinks the binary, as expected. Headroom under the 22,528-byte gate: 2,133 bytes (~9.5%). D-13 STOP rule did NOT trigger. |
| 13-11 | C-M2 + S-M1 gap-closure fixes: `redeem` skips the USDC transfer and returns a (possibly zero) payout instead of reverting `ZeroAmount` on a dust position; `split_by_bps` errors `AllocationInvalid` on an empty weights array with a non-zero amount instead of silently dropping it. Two new unit tests, no new storage or public ABI surface | 20,351 | 1 | −44 vs DROP-PERMIT2-01 (20,395); inside the documented ~100–150 B brotli non-determinism band. Headroom under the 22,528-byte gate: 2,177 bytes (~9.7%). D-13 STOP rule did NOT trigger. |

## vault-periphery (independent 22,528-byte gate)

`vault-periphery` is a brand-new Stylus binary (D-18 Plan B) with its own 22,528-byte / 1-fragment
mainnet gate — it does not share `vault-core`'s budget. Measured from
`packages/contracts/vault-periphery` with `cargo stylus check --endpoint="https://arb1.arbitrum.io/rpc"`.
