...
- **[VERIFIED: `cast code 0x724dc807b04555b71ed48a6896b6F41593b8C637 --rpc-url https://arb1.arbitrum.io/rpc`]** — non-empty bytecode; **[VERIFIED: `cast call 0x724dc807b04555b71ed48a6896b6F41593b8C637 "symbol()(string)"`]** → `"aArbUSDCn"`. This is the rebasing aToken (balance grows in place via `index` rebasing, not via share-price growth) — exactly the PITFALLS.md §6 hazard: an ERC-4626 adapter cannot safely hold a raw aToken because `balanceOf` silently increases outside of transfers, breaking share-accounting invariants. This is why the Stata (`StaticATokenLM`) wrapper is needed as the adapter's actual holding.

  **Correction (2026-07-22, Phase 10 D-11):** this bullet previously cited
  `0x625e7708f30ca75bfd92586e17077590c60eb4cd` (`aArbUSDC`) as the aToken behind this position. That
  is a DIFFERENT aToken: its `UNDERLYING_ASSET_ADDRESS()` resolves to
  `0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8` — bridged **USDC.e**, not native USDC. The aToken for
  the native-USDC reserve this position actually sits in is `aArbUSDCn`
  `0x724dc807b04555b71ed48a6896b6F41593b8C637`. The Stata decision is unaffected (Stata's `asset()`
  was independently verified as native USDC), but the address matters: anything reasoning about the
  underlying aToken — including Phase 10's donation-attack test vector 2 — must use `aArbUSDCn`.
...
**AAVE-REWARDS: DEAD — no claim path needed.** `[VERIFIED: cast call 0x929EC64c34a17401F460460D4B9390518E5B473e "getRewardsData(address,address)(uint256,uint256,uint256,uint256)" 0x724dc807b04555b71ed48a6896b6F41593b8C637 0x912CE59144191C1204E64559FE8253a0e49E6548 --rpc-url https://arb1.arbitrum.io/rpc]` → `distributionEnd = 1719151200` (23 June 2024, more than two years in the past). The non-zero `emissionPerSecond` (`5.01e16`) is a stale value persisted from when the programme was live and is **misleading if read without cross-checking `distributionEnd`**. The 253.7 ARB reported by `getTotalClaimableRewards` is a static remainder accrued by holders of that era; this adapter's position earns nothing. Consequence: no `claim_rewards()` in the adapter (Phase 10 D-06) — there is no yield being forgone. Re-check `distributionEnd` before the production deploy (Phase 15, D-07): if Aave has reactivated an incentive programme, that is when adding a claim path gets decided, with real numbers.
...
[BENCHED 2026-07-21 per the Beefy→Euler swap (D-07) — interface analysis retained as the §3.4 bench entry's backing evidence; no longer an active protocol.]
...
|----------|-----------|-------------------|---------------------|--------------------------|-------------------------|---------------------|
| Aave (via Stata) | `stataArbUSDCn` `0x7cfa...1ad` | `[VERIFIED]` YES — `deposit(uint256,address)`/`redeem(uint256,address,address)`/`convertToAssets` all present, non-1:1 exchange rate | `[CITED]` OZ `Erc4626`-derived Stata implementation rounds down on `convertToShares` (deposit) and down on `convertToAssets` (redeem), the standard 4626-safe direction (protects the vault, not the depositor) — not empirically re-derived here | `[CITED]` Stata inherits Aave Pool's liquidity constraint: `maxRedeem` is capped by the Pool's available liquidity, can be < full balance during high utilization | `[VERIFIED: cast call 0x794a61358D6845594F94dc1DB02A252b5b4814aD "paused()(bool)" --rpc-url https://arb1.arbitrum.io/rpc]` → **reverts**: there is no `Pool.paused()` on Aave v3. Pausing is PER RESERVE, encoded in the configuration bits returned by `getReserveData`/`getReserveConfigurationData` — there is no global protocol flag. Stata reverts when the underlying **reserve** is paused, not when "the Pool" is. Corrected 2026-07-22 (D-12); the prior claim was `[CITED]` and never verified. No code change follows from this: the adapter never checked pause explicitly and does not need to — a paused reserve surfaces as the vault's own `withdraw` reverting, which the adapter propagates without clamping (Phase 9 D-08, covered by `conformance.rs`'s `aave::paused_reserve_propagates_revert`) | Static (non-rebasing) accounting is itself the deviation vs. holding raw `aArbUSDC` directly (PITFALLS.md §6) |
...
| Euler v2 (eUSDC-2) | `0x6afb...673b` | `[VERIFIED]` YES — `deposit(uint256,address)`/`redeem(uint256,address,address)`/`convertToAssets`/`maxRedeem` all present, non-reverting (§3.3) | `[CITED]` OZ-4626-derived round-down convention (not empirically re-derived) | `[CITED]` `maxRedeem` bounded by Euler vault liquidity | `[CITED]` Euler EVK has pause/guardian controls (not cast-verified this pass) | Strictly 4626 end-to-end — no structural deviation (the reason it fits the generic adapter unchanged) |

*Beefy's row moved out with the swap; its verified interface survives in §2.4 and its bench entry in §3.4.*
