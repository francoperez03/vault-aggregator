...
mod tests {
    ...
    #[test]
    fn deposit_with_permit2_rejects_wrong_signature_length() {
        let vm = TestVM::default();
        vm.set_sender(user_addr());
        let mut contract = deploy(&vm);

        // 64 bytes instead of 65 -- no mocks registered, so a pass-through to Permit2 would panic
        // on an unmocked call. Reverting here proves the length check runs first.
        let short_sig: Bytes = alloc::vec![0xABu8; 64].into();
        let result = contract.deposit_with_permit2(
            U256::from(1_000u64),
            U256::from(1u64),
    ...
    #[test]
    fn deposit_with_permit2_rejects_invalid_v() {
        let vm = TestVM::default();
        vm.set_sender(user_addr());
        let mut contract = deploy(&vm);

        let mut bad_sig = alloc::vec![0xABu8; 65];
        bad_sig[64] = 0;
        let result = contract.deposit_with_permit2(
            U256::from(1_000u64),
            U256::from(1u64),
        ...
        assert_eq!(result.unwrap_err(), b"InvalidSignatureV".to_vec());
    }

    #[test]
    fn deposit_with_permit2_rejects_zero_amount() {
        let vm = TestVM::default();
        vm.set_sender(user_addr());
        let mut contract = deploy(&vm);
    ...
    /// Proves `deposit_with_permit2` credits `msg.sender`, not some other address: every mock
    ...
    #[test]
    fn deposit_with_permit2_credits_msg_sender() {
        let vm = TestVM::default();
        ...
        let result = contract.deposit_with_permit2(amount, nonce, deadline, signature);
