//! Live Arbitrum One round-trip against Euler v2 `eUSDC-2`. Spends real USDC (~$3) and gas.
//! Run with:
//!   ARB_ONE_RPC_URL=... M2_WALLET_KEY=... EULER_ADAPTER_ADDR=... cargo test -p adapter-e2e
...
async fn euler_deposit_withdraw_roundtrip() -> anyhow::Result<()> {
    ...
    let adapter_address = euler_adapter_addr()?;
