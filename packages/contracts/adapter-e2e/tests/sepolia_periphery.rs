...
/// `(nonce, deadline, signature)` ready for `depositWithPermit2`. Ported from
...
/// D-19 residual risk #1: after a `depositWithPermit2` completes, the periphery's own USDC
...
async fn periphery_holds_zero_balance_and_allowance_after_deposit() -> anyhow::Result<()> {
    ...
    eprintln!("sending depositWithPermit2...");
    let receipt = periphery
        .depositWithPermit2(amount, nonce, deadline, sig)
        .gas(CORE_TX_GAS_LIMIT)
        .send()
        .await?
        .get_receipt()
    ...
    assert!(
        receipt.status(),
        "depositWithPermit2 reverted on-chain: {}",
    ...
    assert!(
        ...
        "periphery holds {periphery_balance} units of USDC after a completed depositWithPermit2 \
    ...
    assert!(
        ...
        "periphery retains a {periphery_allowance}-unit USDC allowance to the core after \
         depositWithPermit2 -- WR-03 (residual-allowance) must not regress"
    );
    ...
    assert!(
        grew,
        "no adapter's sharesOf(caller) grew after depositWithPermit2 -- the position never \
         landed in the core"
    );
...
/// D-19 residual risk #2: a `depositWithPermit2` whose signer has weights pointing at a disabled
...
#[tokio::test]
async fn deposit_with_permit2_toward_disabled_adapter_reverts_leaving_nothing() -> anyhow::Result<()>
{
    let Some(rpc) = rpc_url() else {
        eprintln!("ARB_SEPOLIA_RPC_URL not set, skipping Sepolia mock-rig test");
        return Ok(());
    ...
    // The Permit2 pull happens INSIDE depositWithPermit2, before the core's own guard fires, so a
    ...
    assert_reverts_with_vault_error(
        ...
        "depositWithPermit2 with weights pointing at a disabled adapter",
    );

    let periphery_balance = usdc.balanceOf(periphery_address).call().await?;
    assert!(
        periphery_balance.is_zero(),
        "periphery holds {periphery_balance} units of USDC after a depositWithPermit2 attempt \
...
async fn permissionless_dust_deposit_for_does_not_disturb_the_offset() -> anyhow::Result<()> {
    ...
    // SAME adapter -- proving the dust gift didn't poison the per-adapter offset for anyone.
    usdc_b
